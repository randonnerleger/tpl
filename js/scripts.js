...
function VideoConsentDisplay(vidid, source, width, height) {
	...
	switch(CookieConsent) {
		...
		default:
			...
			conn.innerHTML= '<div class="inner"><strong>Information relative à vos données personnelles</strong><p>Randonner-Leger.org n\'héberge pas cette vidéo et n\'est responsable ni de son contenu, ni des données personnelles susceptibles d\'être exploitées par son hébergeur lors de sa diffusion (adresse IP, cookies...).</p><div class="consent"><input type="checkbox" name="memorise" id="memorise" value="true">&nbsp;<label for="memorise">Mémoriser mon choix et ne plus afficher ce message.</label></div><input class="" type="submit" onclick="VideoConsentAcceptCookies(\''+vidid+'\', \''+source+'\', \''+width+'\', \''+height+'\');" value="Voir la vidéo" />&nbsp;&nbsp;<input class="" type="submit" id="a_175" id="euCookieAcceptWP" onclick="VideoConsentRefuseCookies(\''+vidid+'\', \''+source+'\');" value="Regarder sur '+source+'" /></div>';
	}
...
function SwitchFontSize(action) {
	...
	document.getElementById('MyCustomCss').innerHTML = '.tclcon a, .pun .postmsg, #punpost .txtarea textarea, #punedit .txtarea textarea, #dokuwiki__site .page {font-size: ' + fontsize + 'rem; line-height: ' + ( (fontsize * 1) + .8) + 'rem !important;}';
